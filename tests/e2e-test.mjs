...
// Email stays OFF unless a section switches it on: dotenv must not leak the
// real Resend key from .env into the suite.
...
/* ═══ 32. The second channel: the overview by email ═══ */
console.log('\n32. The welcome asks for an email; the overview goes out by email');
{
  const { config: cfg } = await import('../src/core/config.js');
  cfg.bot.mode = 'hybrid';
  // Pretend the email provider is configured. No request leaves the process:
  ...
  const ATM = 'Greetings! It was lovely meeting you at ATM Dubai 2026. I am excited to discover more about Vallé Advenature Park and the unforgettable experiences it offers in Mauritius.';
  // A number nobody else in this suite has used.
  ...
  /* ---- with no credentials, email does nothing ---- */
  cfg.email = { apiKey: '', from: '', replyTo: '', bcc: '' };
  const mailBefore2 = mails.length;
